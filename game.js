...
function drawPlayerSprite(ctx, x, y, direction, moving){
  ...
  else { frames = PLAYER_FRAMES.side; mirror = (direction==="right"); } // FIX: sprite dasar (frame side) menghadap KIRI, jadi mirror saat ke KANAN
  const idx = frames[toggle?1:0];
  const sx = idx*PLAYER_CELL_W;
  const drawH = 32, drawW = drawH*(PLAYER_CELL_W/PLAYER_CELL_H); // FITUR: samakan skala dgn NPC di lobby (32px)
  ctx.save();
...
// FIX/FITUR: dinding tak terlihat mengikuti bingkai pohon/batu di gambar latar dungeon —
// area tengah (jalan setapak) bisa dilewati, pinggiran (pohon/batu) tidak, kecuali koridor
// menuju pintu keluar (kiri) & tangga/bos (kanan).
function canMoveInDungeon(px, py, canvas){
  const W = canvas.width, H = canvas.height;
  const insetX = 70, insetTop = 55, insetBottom = 55;
  const inCore = px>insetX && px<W-insetX && py>insetTop && py<H-insetBottom;
  if(inCore) return true;
  if(px<=insetX && Math.abs(py-300)<45) return true; // koridor pintu keluar
  if(px>=W-insetX-15 && py>140 && py<300) return true; // koridor tangga/bos
  return false;
}
...
const LOBBY_BUILDINGS = [
  // Koordinat dikalibrasi presisi dari gambar lobby_bg.png (skala 700x467 canvas dari 1536x1024 asli)
  {name:"Toko Senjata & Armor",  x:160, y:103, w:109, h:68,  doorW:26, action:"shop"},
  {name:"Balai Guild",           x:301, y:84,  w:128, h:114, doorW:30, action:"guild"},
  {name:"Merchant Ramuan",       x:419, y:98,  w:80,  h:78,  doorW:24, action:"merchant"},
  {name:"Gerbang Dungeon",       x:570, y:89,  w:100, h:96,  doorW:30, action:"dungeon_select"},
  {name:"Penginapan",            x:89,  y:193, w:114, h:71,  doorW:24, action:"inn"},
  {name:"Bank",                  x:411, y:194, w:89,  h:68,  doorW:22, action:"bank"},
  {name:"Tukang Besi",           x:510, y:193, w:100, h:71,  doorW:24, action:"blacksmith"},
  {name:"Papan Misi",            x:194, y:310, w:68,  h:46,  doorW:20, action:"questboard"},
  {name:"Pasar",                 x:410, y:310, w:91,  h:64,  doorW:22, action:"market"},
  // Rumah penduduk & air mancur: solid penuh, tidak ada pintu/aksi (sekadar dekorasi tapi tetap menghalangi)
  {name:"Rumah Penduduk (kiri)", x:81,  y:287, w:103, h:64,  doorW:0, action:null},
  {name:"Rumah Penduduk (kanan)",x:586, y:286, w:96,  h:66,  doorW:0, action:null},
  {name:"Air Mancur",            x:280, y:210, w:105, h:100, doorW:0, action:null},
];
...
function buildingDoorZone(b){
  if(!b.doorW) return {x:-9999,y:-9999,w:0,h:0}; // gak ada pintu = dinding solid penuh, gak ada celah
  return {x:b.x, y:b.y+b.h/2-6, w:b.doorW, h:18};
}
...
function drawOverworld(canvas){
  ...
  if(merchantB) drawMonsterSprite(ctx, "wierd_traveler", merchantB.x-16, merchantB.y+merchantB.h/2-14, 30, true);
...
function enterDungeon(dungeonId, subFloor){
  ...
  if(d.finalBoss || isLastFloor){
    ...
    for(let i=0;i<Math.min(remain,8);i++){
      dungeonMonsters.push({ x:170+Math.random()*430, y:90+Math.random()*250, alive:true, isBoss:false, uid:"m"+i, spriteKey: pickMonsterSprite(dungeonId,false) });
    }
    if(remain<=0){
      dungeonMonsters.push({ x:640, y:220, alive:true, isStairs:true, uid:"stairs" });
    }
...
function renderDungeon(){
  ...
  function loop(){
    ...
    if(canMoveInDungeon(nx, player.pos.y, canvas)) player.pos.x = nx;
    let ny = player.pos.y + (keysHeld.down?spd:0) - (keysHeld.up?spd:0);
    ny = Math.max(15, Math.min(canvas.height-15, ny));
    if(canMoveInDungeon(player.pos.x, ny, canvas)) player.pos.y = ny;
    player.isMoving = updateFacingFromKeys(keysHeld);
...
function drawDungeon(canvas, d){
  ...
  if(player.pos.x<45 && Math.abs(player.pos.y-300)<40){
    if(exitArmed){ exitArmed = false; exitDungeonOneLevel(); }
    return;
  }
